...
```bash
cd cam_driver_gscam2
sudo bash install.bash
```

This builds `cam_driver:latest` and registers `camera_stream_0.service` (autostart on boot, same as `robomaster_bridge`).
To stream manually without installing the service: `bash cam_driver_gscam2/run_docker.sh`
...
The `cam_driver_gscam2/` service (JP6 + Raspberry Pi v2 IMX219) runs `gscam2` under `/<robot_ns>/camera_0/`:

| Topic | Type | Description |
|---|---|---|
| `/<robot_ns>/camera_0/image_raw/compressed` | `sensor_msgs/CompressedImage` | JPEG-encoded 1920×1080 @ 30 fps (default, `cam_param_jpg.yaml`) |
| `/<robot_ns>/camera_0/image_raw` | `sensor_msgs/Image` | Raw RGBA 1920×1080 @ 30 fps (`cam_param_raw.yaml`) |
| `/<robot_ns>/camera_0/camera_info` | `sensor_msgs/CameraInfo` | Pi v2 intrinsics (approximate — run `camera_calibration` for accuracy) |

The service runs with `cam_param_jpg.yaml` (JPEG) by default. Edit `cam_driver_gscam2/install.bash` to switch to `cam_param_raw.yaml`.
...
`grab_image.py` (in this repo) subscribes to an image topic and saves one frame to disk.
The default output path is `/home/nvidia/Robot/camera_sample.jpg`.

```bash
# Save the raw full-resolution stream (1920x1080)
python3 /home/nvidia/Robot/grab_image.py /robomaster_2/camera_0/image_raw /home/nvidia/Robot/raw.jpg
```

> **QoS note:** The camera publishes with `reliable` QoS. `grab_image.py` uses `qos_profile_sensor_data` (best-effort), which is compatible with a reliable publisher.
